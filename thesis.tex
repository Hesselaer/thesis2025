...
\begin{abstract}
\noindent
Deterministic finite automata (DFA) are an important tool in computer science with applications ranging from string parsing to checking for patterns in data among others. Additionally, through Kleene's theorem we know the regular languages they describe are exactly the ones defined by regular expressions. Except these regular languages only contain strings of characters, given by sequential composition. This limits their expressive power and excludes systems where concurrent behaviour is inherently present such as software, biology, chemical processes and many others where processes have multiple steps that are dependent in a structure of higher complexity.

Counterparts to DFA and regular expressions, however, have not yet been developed to a satisfactory degree. In a previous attempt higher dimensional automata (HDA) were introduced, except with the exclusion of a repeated parallel operator in its expressions the model is too weak for a variety of applications. We propose a model that aims to solve these problems through a separation of structure and behaviour, as in HDA transitions in the model were restricted by the form of the state space.

To this end, we first propose a type of dependency graph called \emph{ranked hypergraphs}. These are directed acyclic hypergraphs which have root and output interfaces that provide control over dependencies and admit an associative sequential composition. We show that ranked hypergraphs admit a generator which consists of \emph{depth-1 ranked hypergraphs}, which are ranked hypergraphs with paths of at most length 1. We construct a simplicial set of ranked hypergraphs to serve as the counterpart of the alphabet from DFA. Finally, we propose a candidate model of concurrent languages given by coalgebras of an endofunctor on simplicial sets.
\end{abstract}
...
\begin{figure}[H]
    \begin{center}
        \begin{tikzpicture}[scale=0.2]
            \tikzstyle{every node}+=[inner sep=0pt]
            \draw [black] (5.8,-13.6) circle (3);
            \draw (5.8,-13.6) node {$1$};
            \draw [black] (5.8,-28.3) circle (3);
            \draw (5.8,-28.3) node {$2$};
            \draw [black] (20.8,-20.3) circle (3);
            \draw (20.8,-20.3) node {$3$};
            \draw [black] (38.2,-13.8) circle (3);
            \draw (38.2,-13.8) node {$1$};
            \draw [black] (38.2,-28.3) circle (3);
            \draw (38.2,-28.3) node {$2$};
            \draw [black] (55.1,-13.8) circle (3);
            \draw (55.1,-13.8) node {$3$};
            \draw [black] (55.1,-28.3) circle (3);
            \draw (55.1,-28.3) node {$4$};
            \draw [black] (8.54,-14.82) -- (18.06,-19.08);
            \fill [black] (18.06,-19.08) -- (17.53,-18.29) -- (17.13,-19.21);
            \draw (12.38,-17.46) node [below] {$a$};
            \draw [black] (8.45,-26.89) -- (18.15,-21.71);
            \fill [black] (18.15,-21.71) -- (17.21,-21.65) -- (17.68,-22.53);
            \draw (14.3,-24.8) node [below] {$b$};
            \draw [black] (41.2,-13.8) -- (52.1,-13.8);
            \fill [black] (52.1,-13.8) -- (51.3,-13.3) -- (51.3,-14.3);
            \draw (46.65,-14.3) node [below] {$a$};
            \draw [black] (41.2,-28.3) -- (52.1,-28.3);
            \fill [black] (52.1,-28.3) -- (51.3,-27.8) -- (51.3,-28.8);
            \draw (46.65,-28.8) node [below] {$b$};
        \end{tikzpicture}
    \end{center}
    \caption{Dependency graphs of equivalent behaviour ($a$ parallel with $b$)}
    \label{fig:behaviour_equiv}
\end{figure}
\newpage

\section{Conclusions and Further Research}
Ranked hypergraphs seem to be a very strong and intuitive model for concurrent behaviour. Their interfaces allow for more complex dependency handling than standard dependency graphs. Associative composition with a left unit are already very strong properties, which combine nicely with quite a simple generator. Moreover the construction in the proof of \thref{thm_generator} gives a very computable construction of any graph into a sequence of generating depth-$1$ graphs.

More work needs to be done primarily on the semantics side as there isn't yet a working behaviour map. To find this there will need to be changes made to at least the coalgebra, if not the entire simplicial set, as is detailed in section \ref{sec:coalg_problems}.
